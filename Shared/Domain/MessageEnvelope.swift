...
//  PrintParty (Shared)
//
//  Unified message format for WebSocket communication. Mirror of the
//  gateway's MessageEnvelope model — same Codable JSON wire format.
//
//  Used both on LAN (plaintext JSON) and via relay (encrypted).
//  The `payload` field is a base64-encoded JSON string so it can be
//  generically encoded/decoded without type erasure.
//
